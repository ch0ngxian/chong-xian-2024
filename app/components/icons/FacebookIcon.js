export default function FacebookIcon({ className }) {
  return (
    <svg className={className} width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="64" height="64" rx="32" fill="currentColor" />
      <path
        d="M38.21 31.4H34V47H27.75V31.4H24V26.6H27.75V23.708C27.7525 19.4984 29.5737 17 34.74 17H39V21.8H36.1413C34.13 21.8 34 22.52 34 23.8676V26.6H39L38.21 31.4Z"
        fill="white"
      />
    </svg>
  );
}
